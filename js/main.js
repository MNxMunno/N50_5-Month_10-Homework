const cards = document.querySelector(".product__cards");
const loading = document.querySelector(".product__loads");
...
function createCard(data) {
  ...
  data.forEach((product) => {
    ...
    <div class="product__btns">
        <p class="product__card__new">Новинка</p>
        <div class="product__child__btn">
            <img name="btn__like"  class="product__card__heart" src="./images/heart.svg">
            <p class="product__card__scales">
        ...
        <div class="product__card__img">
            <img name='product__hero-img' data-id=${product.id} src="${product.image}"/>
        </div>
        <h3 class="product__card__title" title="${product.title}">${product.title}</h3>
        
        </div>
        <div class="product__card__price-rate">
        <p class="product__card__price">${product.price} $</p>
        <p class="product__card__rating">⭐ ${product.rating.rate}</p>
        </div>
        <div class="product__card__btns">
        <button class="product__card__btn__sale">Купить сейчас</button>
        <button class="product__card__btn__cart">
        <svg class="product__card__btn___cart" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" transform: ;msFilter:;"><path d="M21.822 7.431A1 1 0 0 0 21 7H7.333L6.179 4.23A1.994 1.994 0 0 0 4.333 3H2v2h2.333l4.744 11.385A1 1 0 0 0 10 17h8c.417 0 .79-.259.937-.648l3-8a1 1 0 0 0-.115-.921z"></path><circle cx="10.5" cy="19.5" r="1.5"></circle><circle cx="17.5" cy="19.5" r="1.5"></circle></svg>
        </button>
        </div>
        `;
...
const btnSubmit = document.querySelector(".header__form__btn__submit");
...
let loadingCard = Array(20).fill("");

let fragment = document.createDocumentFragment();
loadingCard.forEach(() => {
  const card = document.createElement("div");
  card.classList.add("skeleton__card");
  card.innerHTML = `
    <div class="skeleton__btns">
                  <div class="skeleton__news skeleton"></div>
                  <div class="skeleton__child__btn">
                    <div class="skeleton__like skeleton"></div>
                    <div class="skeleton__scales skeleton"></div>
                  </div>
                </div>
...
cards.addEventListener("click", (e) => {
  if (e.target.name === "product__hero-img") {
